...
always begin
   ...
   assign 	#(DELAY) DQa = (nG|~oe|ZZ) ? HighZ : bank0[raddr];  // 
   assign 	#(DELAY) DQb = (nG|~oe|ZZ) ? HighZ : bank1[raddr];  // 
   assign 	#(DELAY) DQc = (nG|~oe|ZZ) ? HighZ : bank2[raddr];  // 
   assign 	#(DELAY) DQd = (nG|~oe|ZZ) ? HighZ : bank3[raddr];  // 
   assign 	#(DELAY) DQe = (nG|~oe|ZZ) ? HighZ : bank4[raddr];  // 
   assign 	#(DELAY) DQf = (nG|~oe|ZZ) ? HighZ : bank5[raddr];  // 
   assign 	#(DELAY) DQg = (nG|~oe|ZZ) ? HighZ : bank6[raddr];  // 
   assign 	#(DELAY) DQh = (nG|~oe|ZZ) ? HighZ : bank7[raddr];  // 
   //---------------------------------------------------------------
   //
   //  	Combinatorial Logic 
   //
   ...
   always @ (posedge clk_i) begin: st
	...
	default:
      ...
      if (we) begin 
	 if (bw[0]==1)	bank0[waddr] <= Dia;
	 if (bw[1]==1)	bank1[waddr] <= Dib;
	 if (bw[2]==1)	bank2[waddr] <= Dic;
	 if (bw[3]==1)	bank3[waddr] <= Did;
	 if (bw[4]==1)	bank4[waddr] <= Die;
	 if (bw[5]==1)	bank5[waddr] <= Dif;
	 if (bw[6]==1)	bank6[waddr] <= Dig;
	 if (bw[7]==1)	bank7[waddr] <= Dih;
